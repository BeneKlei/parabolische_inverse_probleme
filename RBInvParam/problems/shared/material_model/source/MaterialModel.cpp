// MaterialModel.cpp (refactored: no `using namespace dealii;`)
// C++
#include <cmath>
#include <cstdint>
#include <filesystem>
...
MaterialModel::MaterialModel(const MaterialModelBaseConfig &config)
  ...
  // ---------------------- State FE ----------------------
  , m_state_fe(dealii::FE_Q<dim>(1), dim)
  , m_state_dof_handler(m_state_triangulation)
  , m_state_quadrature(dealii::QGaussLobatto<dim>(2))
  // NOTE: m_state_mapping, m_state_sp, m_BC_constraints are default-constructed members

  // ---------------------- Param FE ----------------------
  , m_param_fe(dealii::FE_Q<dim>(1))
  , m_param_dof_handler(m_param_triangulation)
  , m_param_quadrature(dealii::QGaussLobatto<dim>(2))
  // NOTE: m_param_mapping, m_param_sp, m_param_constraints, m_param_free_dofs are default-constructed
  ...
  , m_param_space_context(
        m_param_triangulation,              // param triangulation
        m_param_fe,
        m_param_dof_handler,
        m_param_mapping,                    // param mapping
...
void MaterialModel::setup_param_space()
{
  ...
  std::vector<dealii::Point<dim>> support_points(m_param_dof_handler.n_dofs());
  ...
  for (dealii::types::global_dof_index i = 0; i < support_points.size(); ++i)
  ...
  dealii::DoFTools::make_sparsity_pattern(m_param_dof_handler, m_param_sp);
  m_param_sp.compress();
}
...
void MaterialModel::setup_state_space()
{
  ...
  m_state_sp.reinit(m_state_dof_handler.n_dofs(),
                    m_state_dof_handler.n_dofs(),
                    m_state_dof_handler.max_couplings_between_dofs());
  dealii::DoFTools::make_sparsity_pattern(m_state_dof_handler, m_state_sp);
  m_state_sp.compress();
...
void MaterialModel::setup_system()
{
  ...
  m_body_force = m_body_force_factory.assemble_body_force(ctx_body_force);
...
void MaterialModel::assemble_force(dealii::Vector<Number> &result, double time)
{
  Assert(result.size() == m_state_dof_handler.n_dofs(),
         dealii::ExcDimensionMismatch(result.size(), m_state_dof_handler.n_dofs()));

  dealii::QGaussLobatto<dim> quadrature_formula(2);
  ...
  std::vector<dealii::Vector<double>> body_force_values(n_quadrature_points,
                                                        dealii::Vector<double>(dim));

  result = 0;
  m_body_force->set_time(time);

  auto cell = m_state_dof_handler.begin_active();
  const auto endc = m_state_dof_handler.end();
...
void MaterialModel::assemble_force_list()
{
  ...
  for (std::uint32_t idx = 0; idx <= static_cast<std::uint32_t>(m_base_config.nt); ++idx)
  {
...
) const
{
  ...
  return std::make_unique<MaterialModel::SparMatOp>(std::move(prod_C_mat));
}
...
) const
{
  ...
  return std::make_unique<MaterialModel::SparMatOp>(std::move(obs_op_mat));
}

void MaterialModel::clear_rhs_boundary_dofs(dealii::Vector<Number> &v)
{
...
) const
{
  ...
  buf.reinit(buf_sp);
  product_C_mat.mmult(buf, obs_op_mat, dealii::Vector<Number>(), false);

  dealii::SparsityPattern buf_sp_ = utils::make_product_sparsity_ATB(obs_op_mat, buf);
  m_bilinear_cost_operator_sp.copy_from(buf_sp_);
  bilinear_cost_operator.reinit(m_bilinear_cost_operator_sp);

  obs_op_mat.Tmmult(bilinear_cost_operator, buf, dealii::Vector<Number>(), false);

  return std::make_unique<MaterialModel::SparMatOp>(std::move(bilinear_cost_operator));
}

void MaterialModel::save_state(const dealii::Vector<Number> &v,
                               const std::string &save_path)
{
  ...
  std::vector<dealii::DataComponentInterpretation::DataComponentInterpretation> dci(3);
  for (unsigned int i = 0; i < 3; ++i)
    dci[i] = dealii::DataComponentInterpretation::component_is_part_of_vector;

  data_out.attach_dof_handler(m_state_dof_handler);
  data_out.add_data_vector(v, solution_names, dealii::DataOut<3>::type_dof_data, dci);
...
void MaterialModel::save_time_series(const std::vector<dealii::Vector<double>> &v,
                                     const std::string &name,
                                     const std::string &save_path,
                                     const std::vector<double> &times)
{
  // ensure output directory exists
  const std::filesystem::path dir = std::filesystem::path(save_path) / name;
  if (!std::filesystem::exists(dir))
    std::filesystem::create_directories(dir);

  dealii::DataOut<3> data_out;

  const std::vector<std::string> solution_names = {"x", "y", "z"};
  ...
  {
    data_out.attach_dof_handler(m_state_dof_handler);
    data_out.add_data_vector(v[t], solution_names, dealii::DataOut<3>::type_dof_data, dci);
    data_out.build_patches();
