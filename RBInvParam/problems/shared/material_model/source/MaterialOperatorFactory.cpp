...
template class MaterialOperatorFactory<3, double>;

template <int dim, typename Number>
void MaterialOperatorFactory<dim, Number>::assemble_system(
  const MaterialOperatorFactoryContext<dim, Number>& ctx,
  ...
  bool &affine) const
{
  ...
  case MaterialOperatorType::CosseratSpatial:
    ...
    break;
  default:
    throw std::runtime_error("Unknown MaterialOperatorType.");
  }
}

template <int dim, typename Number>
void MaterialOperatorFactory<dim, Number>::assemble_cosserat_system(
  const MaterialOperatorFactoryContext<dim, Number>& ctx,
  std::vector<SparseMatrix> &matrices,
  bool &affine) const
{
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
    {
      ...
      for (unsigned int j = 0; j < dofs_per_cell; ++j)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
        {
          ...
          const double sym_term =
              grad_i[component_j] * grad_j[component_i] +
              ((component_i == component_j) ? grad_i * grad_j : 0.0);
          const double skew_term =
              grad_i[component_j] * grad_j[component_i] -
              ((component_i == component_j) ? grad_i * grad_j : 0.0);
...
template <int dim, typename Number>
void MaterialOperatorFactory<dim, Number>::assemble_cosserat_spatial_system(
  const MaterialOperatorFactoryContext<dim, Number>& ctx,
  std::vector<SparseMatrix> &matrices,
  bool &affine) const
{
...
template <int dim, typename Number>
void MaterialOperatorFactory<dim, Number>::assemble_cosserat_delamination_system(
  const MaterialOperatorFactoryContext<dim, Number>& ctx,
  std::vector<SparseMatrix> &matrices,
  bool &affine) const
{
